...
export class ScannerSystem extends createSystem(
  ...
  update(delta: number, time: number) {
    ...
    // Track which entities have been scanned this frame to avoid
    // double-processing (e.g. if multiple scanner zones overlap).
    ...
    for (const scanner of this.queries.scanners.entities) {
      ...
      for (const scannable of this.queries.scannables.entities) {
        // Already handled this entity in this frame?
        if (scannedThisFrame.has(scannable)) {
          continue;
        }
        ...
        // Mark this entity as scanned this frame so no other scanner
        // (or second pass) can process it again.
        ...
        // IMPORTANT: only process one scannable per scanner per frame
        // to avoid multiple respawns when several items overlap the zone.
        break;
      }
    }
