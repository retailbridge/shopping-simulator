...
export class ScannerSystem extends createSystem(
  ...
  // --- Product display info (for toast) ---
  private productInfo: Record<
    string,
  ...
  > = {
    ...
    chickenStrips: {
      name: "Crispy Chicken Strips (8 pcs)",
      priceLabel: "$8.49",
    },
    fleeceJacket: {
    ...
    glasses: {
      name: "Classic Eyeglasses Frame",
      priceLabel: "$149.00",
    },
    goldBar: {
      name: "Gold Bar (Display)",
      priceLabel: "$29.99",
    },
    industrialShoe: {
      name: "Industrial Work Boot",
      priceLabel: "$129.99",
    },
    presentBox: {
      name: "Gift Box",
  ...
  // Build the label text for the toast.
  // Falls back to the original "Scanned - Good Job!" style if no info.
  private buildToastMessage(productId: string): string {
    const info = this.productInfo[productId];
    if (info) {
      return `${info.name} — ${info.priceLabel}`;
    }
    if (productId) {
      return `Scanned: ${productId}`;
    }
    return "Scanned - Good Job!";
  }

  private showScanToast(message?: string) {
    if (!this.toastEntity || !this.toastEntity.object3D) return;

    const label = message ?? "Scanned - Good Job!";

    // Update the PanelUI text if the element exists.
    // Expect scan-toast.json to have an element with id "scanLabel".
    try {
      const panel = this.toastEntity.getComponent(PanelUI) as any;
      const doc = panel?.uiDocument;
      const labelEl = doc?.getElementById?.("scanLabel");
      if (labelEl && typeof labelEl.setProps === "function") {
        labelEl.setProps({ value: label });
      }
    } catch {
      // If UI doc isn't ready, just show the panel with whatever default text it has.
    }
  ...
  update(delta: number, time: number) {
    ...
    for (const scanner of this.queries.scanners.entities) {
      ...
      for (const scannable of this.queries.scannables.entities) {
        ...
        const toastMessage = this.buildToastMessage(productId);
        this.showScanToast(toastMessage);
    ...
    if (respawns.length > 0) {
      ...
      for (const r of respawns) {
        const key = `${r.homeX.toFixed(3)}|${r.homeY.toFixed(3)}|${r.homeZ.toFixed(3)}`;
