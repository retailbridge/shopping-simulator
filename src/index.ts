...
const assets: AssetManifest = {
  ...
  // Level complete (legacy, kept in case)
  levelWinSound: {
    url: "./audio/levelwin.mp3",
    type: AssetType.Audio,
    priority: "background",
  },

  // Timer warning (10s remaining)
  timerSound: {
    url: "./audio/timer.mp3",
    type: AssetType.Audio,
    priority: "background",
  ...
  // Win / lose sounds for the round
  youWinSound: {
    url: "./audio/youwin.mp3",
    type: AssetType.Audio,
    priority: "background",
  },
  youLoseSound: {
    url: "./audio/youlose.mp3",
    type: AssetType.Audio,
    priority: "background",
  },
  ...
  // POS / cash register model
  pointOfSale: {
    url: "./gltf/point-of-sale/model.gltf",
    type: AssetType.GLTF,
    priority: "normal",
  },
};
...
const PRODUCT_POOL: ProductConfig[] = [
  ...
  { assetKey: "goldBar", productId: "goldBar", scale: 0.25 },
  ...
  { assetKey: "fleeceJacket", productId: "fleeceJacket", scale: 1.6 },
...
const DESK_SLOTS = [
  { x: -0.55, y: 0.87, dz: -0.05 }, // left  (was -0.25)
  { x: -0.20, y: 0.87, dz: -0.05 }, // center (was  0.10)
  { x:  0.10, y: 0.90, dz: -0.10 }, // right (was  0.40)
];
...
World.create(document.getElementById("scene-container") as HTMLDivElement, {
  ...
  .then((world) => {
    ...
    // Play welcome sound when the world loads
    const welcomeAudioEntity = world
    ...
    AudioUtils.play(welcomeAudioEntity);
    ...
    //
    // POS / CASH REGISTER – beside the scanner, larger and angled toward player
    //
    const posGltf = AssetManager.getGLTF("pointOfSale");
    if (posGltf) {
      const posMesh = posGltf.scene.clone(true);

      // Position: a bit to the left of the scanner and slightly back
      posMesh.position.set(0.45, 0.82, deskFrontZ - 0.60);

      // Make it about 3× larger than before (good desk presence)
      posMesh.scale.setScalar(0.9);

      // Rotate so the front of the POS faces the player at ~45°
      // (flip 180° if it was showing the back, then angle toward center)
      posMesh.rotation.y = -Math.PI / 4;

      world.createTransformEntity(posMesh);
    } else {
      console.warn("[index] pointOfSale GLTF not found");
    }
