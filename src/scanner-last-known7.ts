...
export class ScannerSystem extends createSystem(
  ...
  private levelWinAudioEntity: any | null = null;
  ...
  private readonly goalCartValue = 25; // $25 goal
  ...
  init() {
    ...
    // Level win sound on round complete
    this.levelWinAudioEntity = this.world
      .createTransformEntity()
      .addComponent(AudioSource, {
        src: "./audio/levelwin.mp3",
        maxInstances: 1,
        playbackMode: PlaybackMode.FadeRestart,
      });
  ...
  private setupScoreTextEntity() {
    ...
    const geo = new PlaneGeometry(0.40, 0.09);
    const mat = new MeshBasicMaterial({
      map: texture,
      transparent: true,
      opacity: 1.0,
  ...
  private updateScoreTextTexture(count: number, total: number) {
    ...
    const label = `Scanned: ${count} | Total: $${total.toFixed(2)}`;
  ...
  private updateRoundMessageTexture(score: number, total: number) {
    ...
    ctx.fillStyle = "#ffffff";
    ctx.fillText("Time's up!", centerX, baseY - 60);
    ...
    ctx.fillText(
      `Cart total: $${total.toFixed(2)} (Goal: $${this.goalCartValue.toFixed(2)})`,
      centerX,
      baseY + 20,
    );
    ...
    ctx.fillText(
      hitGoal ? "Nice job! You hit the goal." : "Try again to hit $25.00.",
      centerX,
      baseY + 60,
    );
  ...
  private drawOnboardingMessage() {
    ...
    ctx.font =
      "normal 28px system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
    ctx.fillText(
      "You have 30 seconds. Hit $25 to win.",
      centerX,
      baseY + 40,
    );
  ...
  private playLevelWinSound() {
    if (this.levelWinAudioEntity) {
      AudioUtils.play(this.levelWinAudioEntity);
    }
  }
  ...
  private showRoundComplete() {
    ...
    if (cam && cam.position) {
      ...
      const dist = 1.4; // about 1.4m in front of user
      anchorPos = cam.position.clone().add(forward.multiplyScalar(dist));
      anchorPos.y = cam.position.y; // eye-level
    } else {
    ...
    // Play win sound once per round (game over)
    ...
    if (
      ...
      msgObj.position.y += 0.25; // slightly above the panel
  ...
  update(delta: number, time: number) {
    ...
    // Keep onboarding hint in front of the user until hidden
