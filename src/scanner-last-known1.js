 ...
 *  - when timer ends, scanning stops and a round-complete panel is shown
 *  - "Play Again" lets you restart the round (timer + score reset)
 */
export class ScannerSystem extends createSystem(
  ...
  private setupRoundCompletePanel() {
    ...
    if (obj) {
      ...
      // Roughly in front of the desk; tweak as needed
      obj.position.set(0, 1.6, 0.2);
    }

    // Best-effort wiring of the "Play Again" button
    try {
      const panel = this.roundPanelEntity.getComponent(PanelUI) as any;
      const doc = panel?.uiDocument;
      if (doc && typeof doc.addEventListener === "function") {
        doc.addEventListener("click", (evt: any) => {
          const targetId =
            evt?.target?.id ??
            evt?.detail?.id ??
            evt?.detail?.targetId ??
            "";
          if (targetId === "playAgainButton") {
            this.resetRound();
          }
        });
      }
    } catch {
      // Safe no-op if PanelUI API differs
    }
  }
  ...
  private showRoundComplete() {
    ...
    if (!this.roundPanelEntity) return;
    const obj = this.roundPanelEntity.object3D;
    if (!obj) return;

    // Position the panel above the first scanner zone we find
    ...
    if (anchorPos) {
      ...
      obj.position.y += 0.45; // slightly above the scanner tile
    } else {
      // Fallback: keep a sensible default if no scanner found
      obj.position.set(0, 1.6, 0.2);
    }
    ...
    // Try to push score into the UI if supported
    try {
      const panel = this.roundPanelEntity.getComponent(PanelUI) as any;
      const doc = panel?.uiDocument;
  ...
  update(delta: number, time: number) {
    ...
    for (const scanner of this.queries.scanners.entities) {
      ...
      for (const scannable of this.queries.scannables.entities) {
        ...
        // If the round is over, ignore new scans
        if (!this.timerRunning && this.timeRemaining <= 0) {
          continue;
        }

        // First ever successful scan: start the round timer
        if (!this.gameStarted) {
