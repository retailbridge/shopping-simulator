...
export class ScannerSystem extends createSystem(
  ...
  update(delta: number, time: number) {
    ...
    for (const scanner of this.queries.scanners.entities) {
      ...
      // NEW: limit to a single scan per scanner per update() call
      let scannedThisUpdate = false;

      for (const scannable of this.queries.scannables.entities) {
        if (scannedThisUpdate) {
          break;
        }
        ...
        // Ensure we don't process any more items this frame for this scanner
        scannedThisUpdate = true;
      }
    }
